...
struct BackgroundTypeCard: View {
    ...
    private let logger = Logger(subsystem: "com.sight.app", category: "Appearance")
    ...
    private func handleDrop(providers: [NSItemProvider]) -> Bool {
        ...
        provider.loadItem(forTypeIdentifier: "public.file-url", options: nil) { item, error in
            if let error = error {
                logger.error("[SightAppearanceView] Drop failed: \(error.localizedDescription)")
                return
            }
            ...
            guard let data = item as? Data,
                ...
                logger.error("[SightAppearanceView] Invalid drop: not an image file")
                return
            }
