...
@MainActor struct SightFlashReminderView: View {
        ...
        VStack(alignment: .leading, spacing: SightTheme.sectionSpacing) {
            ...
            SettingsCard {
                SettingsToggleRow(
                    ...
                    description:
                        "Gentle screen flashes that catch your attention without disrupting your flow. Tailor these gentle alerts to remind you to adjust your posture, sit up straight, or any other habit you want to reinforce.",
            ...
            SettingsCard {
                LargeValueSlider(
                    ...
                    description:
                        "The duration in milliseconds that the flash reminder is presented on screen",
                    value: Binding(
                        get: { Double(preferences.flashDurationMs) },
                        set: { preferences.flashDurationMs = Int($0) }
                    ),
                    range: 400...900,
                    step: 50,
                    unit: "milliseconds",
            ...
            SettingsCard {
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    HStack {
                        ...
                        HStack(spacing: 8) {
                            ForEach(["cyan", "green", "orange", "white"], id: \.self) { color in
                                Circle()
                                    .fill(colorFor(color))
                                    ...
                                    .overlay(
                                        Circle()
                                            .stroke(
                                                preferences.flashColor == color
                                                    ? Color.white : Color.white.opacity(0.3),
                                                lineWidth: preferences.flashColor == color ? 2 : 1
                                            )
                                    )
                                    .onTapGesture {
                                        preferences.flashColor = color
    ...
    private func colorFor(_ name: String) -> Color {
        switch name {
        case "cyan": return SightTheme.accent
