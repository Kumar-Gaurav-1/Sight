...
enum SightTheme {
    ...
    static let breathingAnimation = Animation.easeInOut(duration: 4).repeatForever(autoreverses: true)
...
extension View {
    ...
    func glassMorphism() -> some View {
        self
            ...
            .overlay(
                RoundedRectangle(cornerRadius: SightTheme.cardCornerRadius)
...
struct HoverableCardModifier: ViewModifier {
    ...
    func body(content: Content) -> some View {
        content
            ...
            .shadow(color: isHovered ? SightTheme.shadowMedium : SightTheme.shadowSoft, 
...
struct SightSecondaryButtonStyle: ButtonStyle {
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .padding(.vertical, 6)
            .background(isHovered ? SightTheme.elevatedBackground.opacity(1.2) : SightTheme.elevatedBackground)
            .cornerRadius(6)
            .overlay(
                RoundedRectangle(cornerRadius: 6)
                    .stroke(isHovered ? SightTheme.accent.opacity(0.5) : SightTheme.border, lineWidth: 1)
            )
            .scaleEffect(configuration.isPressed ? 0.97 : 1.0)
            .animation(SightTheme.springSnappy, value: configuration.isPressed)
            .onHover { hovering in
        ...
        HStack {
            ...
            ZStack {
                RoundedRectangle(cornerRadius: 16)
                    .fill(configuration.isOn ? SightTheme.accentGradient : LinearGradient(colors: [SightTheme.elevatedBackground], startPoint: .leading, endPoint: .trailing))
                    .frame(width: 44, height: 26)
                    .shadow(color: configuration.isOn ? SightTheme.accent.opacity(0.3) : .clear, radius: 4, x: 0, y: 0)
...
struct ProgressRing: View {
    ...
    init(progress: Double, lineWidth: CGFloat = 4, gradient: LinearGradient = SightTheme.accentGradient) {
