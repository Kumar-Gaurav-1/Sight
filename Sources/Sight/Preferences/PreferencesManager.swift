...
public final class PreferencesManager: ObservableObject {
    ...
    public init(defaults: UserDefaults = .standard) {
        ...
        let rawBlinkInterval = defaults.object(forKey: Keys.blinkReminderInterval) as? Int ?? 120  // 2 minutes default
        self.blinkReminderIntervalSeconds = min(max(rawBlinkInterval, 10), 3600)
        ...
        self.flashDurationMs = min(max(rawFlashDuration, 100), 2000)
        ...
        let rawPostureInterval = defaults.object(forKey: Keys.postureReminderInterval) as? Int ?? 25 * 60
        self.postureReminderIntervalSeconds = min(max(rawPostureInterval, 60), 7200)
        ...
        let rawLongBreakDuration = defaults.object(forKey: Keys.longBreakDurationSeconds) as? Int ?? 5 * 60  // 5 minutes default
        self.longBreakDurationSeconds = min(max(rawLongBreakDuration, 60), 3600)
        ...
        if let rawMessages = defaults.array(forKey: Keys.breakCustomMessages) as? [String] {
            self.breakCustomMessages = rawMessages
        } else {
            self.breakCustomMessages = []
        }
