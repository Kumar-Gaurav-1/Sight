...
public final class AdherenceManager: ObservableObject {
    ...
    public func getWeeklySummary() -> WeeklySummary {
        ...
        let bestDayName = bestDayStats.map { SharedFormatters.shared.fullDayOfWeek.string(from: $0.date) } ?? "N/A"
    ...
    public func exportAsCSV() -> String {
        ...
        for day in stats.sorted(by: { $0.date < $1.date }) {
            csv += "\(SharedFormatters.shared.yyyyMMdd.string(from: day.date)),"
    ...
    public func exportToCSV() -> String {
        ...
        for day in stats.sorted(by: { $0.date < $1.date }) {
            let line =
                "\(SharedFormatters.shared.iso8601.string(from: day.date)),\(day.breaksCompleted),\(day.breaksSkipped),\(day.nudgesFollowed),\(day.nudgesSnoozed),\(day.totalBreakMinutes),\(String(format: "%.1f", day.dailyScore))\n"
            csv += line
        }
