...
public final class AdherenceManager: ObservableObject {
    ...
    public func exportAsJSON() -> Data? {
        let exportData: [String: Any] = [
            // Performance: Use cached SharedFormatters
            "exportDate": SharedFormatters.shared.isoFormatter.string(from: Date()),
            "version": 1,
            "summary": [
            ...
            "days": stats.map { day -> [String: Any] in
                [
                    // Performance: Use cached SharedFormatters
                    "date": SharedFormatters.shared.isoFormatter.string(from: day.date),
    ...
    public func exportToCSV() -> String {
        ...
        for day in stats.sorted(by: { $0.date < $1.date }) {
            // Performance: Use cached SharedFormatters
            let line =
                "\(SharedFormatters.shared.isoFormatter.string(from: day.date)),\(day.breaksCompleted),\(day.breaksSkipped),\(day.nudgesFollowed),\(day.nudgesSnoozed),\(day.totalBreakMinutes),\(String(format: "%.1f", day.dailyScore))\n"
            csv += line
        }
    ...
    public func saveExport(format: ExportFormat) -> URL? {
        ...
        // Performance: Use cached SharedFormatters
        let timestamp = SharedFormatters.shared.isoFormatter.string(from: Date())
