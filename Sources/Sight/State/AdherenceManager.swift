...
public final class AdherenceManager: ObservableObject {
    ...
    public func getWeeklySummary() -> WeeklySummary {
        ...
        let bestDayName = bestDayStats.map { Formatters.dayFormatter.string(from: $0.date) } ?? "N/A"
    ...
    public func exportAsJSON() -> Data? {
        let exportData: [String: Any] = [
            "exportDate": Formatters.iso8601Formatter.string(from: Date()),
            "version": 1,
            "summary": [
            ...
            "days": stats.map { day -> [String: Any] in
                [
                    "date": Formatters.iso8601Formatter.string(from: day.date),
    ...
    public func exportAsCSV() -> String {
        ...
        for day in stats.sorted(by: { $0.date < $1.date }) {
            csv += "\(Formatters.dateFormatter.string(from: day.date)),"
    ...
    public func exportToCSV() -> String {
        ...
        let formatter = Formatters.iso8601Formatter
    ...
    public func saveExport(format: ExportFormat) -> URL? {
        ...
        let timestamp = Formatters.iso8601Formatter.string(from: Date())
