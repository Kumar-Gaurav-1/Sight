...
final class MicroNudgesTests: XCTestCase {
    ...
    func testNudgeTypeDefaultIntervals() {
        XCTAssertEqual(NudgeType.blink.defaultInterval, 20)
    ...
    func testEscalationActionCodable() throws {
        for action in [EscalationPolicy.EscalationAction.suggestLongerBreak,
                       .forceBreak, .notify] {
            let data = try JSONEncoder().encode(action)
            let decoded = try JSONDecoder().decode(EscalationPolicy.EscalationAction.self, from: data)
